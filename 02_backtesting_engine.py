...
df = pd.read_csv("brvm_historique.csv", encoding="utf-8-sig", sep=";")
...
df["Clôture"] = pd.to_numeric(df["Clôture"].astype(str).str.replace("\xa0", "").str.replace(" ", ""), errors="coerce")
...
plt.tight_layout()
plt.savefig("backtest_zscore_FTSC.png")
plt.show()
print("Graphique sauvegarde")
git add .
git commit -m "Projet 02 - Backtesting Engine Mean-Reversion Z-score BRVM"
git push
